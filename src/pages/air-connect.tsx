      ...
      {section === "Overview" && <div className="air-connect-grid">
        <section className="air-connect-runbook">
          ...
          <div className="air-connect-steps">
            ...
            <article data-complete={Boolean(node)}><i>02</i><div><b>Register the edge gateway</b><span>Vendor-neutral adapter for signed QoS, VLAN, client count, and metering commands.</span></div><button className="button secondary compact" disabled={Boolean(node) || Boolean(working)} onClick={() => void act("Registering edge", "register_edge_node", { name: "AMX Edge Gateway 001", nodeId: "edge-hub-001", locationId: "hub-001", adapterType: "generic-http" })}>{node ? <CheckCircle2/> : <Router/>}{node ? "Registered" : "Register"}</button></article>
