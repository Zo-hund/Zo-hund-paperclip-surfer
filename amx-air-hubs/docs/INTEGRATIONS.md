...
Viewer tokens set `canSubscribe: true`, `canPublish: false`, and `canPublishData: false`; they never dispatch a room agent. Keep the operator app behind authenticated or owner-only access. A truly anonymous viewer URL must be deployed on an isolated public hostname that exposes only the watch surface and subscribe-only token route. Do not make the current all-routes Stage project public as a shortcut.
