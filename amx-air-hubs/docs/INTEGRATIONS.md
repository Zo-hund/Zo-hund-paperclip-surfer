...
The public viewer exposes an explicit Listen Live gate because mobile Safari and Chrome block remote audio until a user gesture. It calls `Room.startAudio()`, resumes every attached LiveKit audio element, and starts a receive-only local Web Audio venue mix from synchronized Stage state. Audio tracks that arrive after the listener unlocks playback start automatically; the venue mix remains audible if a remote producer track is absent or silent. Every room used by `/watch/:room` must be listed in `PUBLIC_LIVEKIT_ROOMS`.

The viewer disables LiveKit adaptive video suspension and attaches the selected `RemoteVideoTrack` directly to both the full-frame Program video and the Three.js `VideoTexture`. This keeps the center venue screen moving when the audience switches from Program to Venue mode instead of freezing on the last decoded frame.
...
The Run console adds one governed workflow across pre-production, live production, and post-production. Pre-production cannot be locked ready until all required source/comms/rights/accessibility/safety checks, department assignments, and editorial/technical/talent/sponsor/safety approvals are complete. The timed rundown can target `MAIN-STAGE` or any connected Pod; taking an item updates its lifecycle evidence and drives the same synchronized Stage cue, camera shot, sponsor, and Pod source used by the other production controls. Director hold/resume, current/next status, planned-versus-elapsed clocks, post-show deliverables, handoff notes, and the bounded activity trail are included in the shared room packet.

The same workflow is debounced into D1 under tenant plus room through `/api/stage/workflows/:roomCode`. D1 revision checks protect operator handoffs from stale saves; Supabase remains the low-latency cue bus. If durable storage or the operator-host policy is unavailable, the console labels itself `LOCAL` and continues using synchronized room state without implying that an enterprise record was saved. Set `STAGE_OPERATOR_HOSTS` to every private Stage hostname, or rely on the `LIVEKIT_OPERATOR_HOSTS` fallback. Exported JSON uses the `amx.stage.production-report.v1` schema and includes gates, crew, rundown timing, deliverables, notes, and activity evidence.
...
Viewer tokens set `canSubscribe: true`, `canPublish: false`, and `canPublishData: false`; they use server-generated identities, require a `PUBLIC_LIVEKIT_ROOMS` match, and never dispatch a room agent. Publisher tokens require both an active operator account and a `LIVEKIT_OPERATOR_HOSTS` match. The public hostname exposes the watch, account, published profile, sponsor, scan, and invite surfaces; route guards and Worker authorization protect member and operator functions.
...
The Audio console synchronizes show, podcast, and DJ formats; transport; Deck A/B sources; equal-power crossfader; BPM; master level; soundscape; stinger cue; and podcast record cue. Operators must affirm broadcast rights before uploading an audio file. Accepted audio is capped at 25 MB, stored in private R2 through `/api/media`, and represented in room state by bounded metadata and a same-origin media ID. The library supports preview, Deck A/B assignment, stinger assignment, and deletion. Its shared `startedAt` value aligns uploaded loop playback across enabled clients. Each browser must explicitly enable its local monitor because Web Audio autoplay policy is device-local. `START RECORD CUE` is synchronized rundown state, not a claim that a server-side podcast file is being archived.

The Stage Web Audio engine routes decks and stingers through a program bus, spatial ambience through a separate atmosphere bus, then applies master gain, mute, and dynamics compression before a `MediaStreamAudioDestinationNode`. Starting the program creates an audio-only LiveKit producer connection and publishes that stereo stream as `AMX Program Mix`; camera permission is not required for the mix itself. Voice capture uses a 48 kHz mono path with browser echo cancellation, noise suppression, automatic gain control, voice isolation when available, high-pass filtering, presence EQ, compression, operator gain, and a 96 kbps LiveKit preset. The studio mixer exposes real voice, program, ambience, and master switches, levels, and meters. Only one Stage operator should publish the program mix to avoid doubled audio. The `DJ LIVE / RTMP` console starts and stops a LiveKit RoomComposite egress for the active Stage room. Configure up to five comma- or newline-separated RTMP/RTMPS ingest URLs in the Worker-only `DJ_RTMP_URLS` secret and set a separate `DJ_STREAM_CONTROL_TOKEN`. The browser keeps the entered control token only in component memory and never receives destination URLs. The Three.js/WebGPU viewport still requires a camera or screen track when it must appear in the outgoing composite.

Nexus Pod view includes a `ROOM TO STAGE` console. `LIVE EVENT` and `PODCAST` open a new Stage operator tab scoped to the same bounded LiveKit room, keep the Nexus room and its participants running, select the Stage Audio console, and apply the matching sound format. `PUBLIC VIEWER` opens `/watch/:roomCode`. The Stage operator then starts the program mix and protected RoomComposite egress; the Nexus cameras, voices, screen shares, and Stage program bus are all part of that room output.

The synchronized video profile defaults to `1080p30` and offers `720p30`, `1080p30`, and `1080p60` for capture and egress independently. Stage camera publishing requests the selected resolution, uses a 1080 primary simulcast layer with 540p and 216p fallbacks, and republishes the local track when an active operator changes capture profile. The hidden Stage router and branded viewer explicitly request LiveKit's high video layer. The Show console reads `MediaStreamTrack.getSettings()` and reports the actual delivered source; a 1080 request is never presented as 1080 when the browser or hardware supplies less. Use 1080p30 for normal production. Use 1080p60 only when the camera, publisher uplink, LiveKit plan, egress capacity, and destination all sustain it.

The Show Score is part of the same revisioned Stage packet. An operator can run it manually or arm Cue Follow so Standby, Opening, Speaker, Demo, Q&A, Sponsor, and Close select coordinated lighting, visual, sound, and intensity presets whenever a show or rundown cue is taken. All clients derive bar, beat, and visual phase from the synchronized `firedAt` timestamp plus the bounded 60-160 BPM transport. The Three.js rig renders bright house/key lights, four moving fixtures, beat rings, laser sweeps, prism halos, and a lightweight finale confetti field. Procedural impact, riser, pulse, and sparkle transitions enter the existing compressed LiveKit program mix. No strobe preset is exposed; motion-reduction settings freeze moving effects while preserving the selected look. Scene health exposes `data-score-armed`, `data-score-mode`, `data-score-cue`, `data-score-lighting`, `data-score-vfx`, `data-score-sound`, and `data-score-beat`.
