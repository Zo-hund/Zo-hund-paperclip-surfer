...
23. In Audio > Show Score, enable the local monitor, arm the score, keep Cue Follow selected, and take every Show and rundown cue. Confirm the lighting, VFX, transition sound, cue name, and `data-score-*` values change together on a second Stage client. Exercise all six lighting looks and five visual designs manually, verify the bar/beat clock advances at the selected BPM, and confirm reduced-motion mode freezes moving effects. Check that no control exposes rapid strobing, then test at 320x568, 390x844, 844x390, and desktop with a nonblank venue canvas.
...
25. In Show > Video Production, select `1080P30`, connect the camera, and confirm Actual Input reports `1920x1080` when the device supports it. Route that participant to a camera channel and verify the hidden router and `/watch/:roomCode` expose a nonvirtual program feed. In Audio > DJ LIVE, select each output quality and confirm start sends only LiveKit presets `0`, `2`, or `3`. Use 1080p30 for the release test; validate 1080p60 only on a wired or measured uplink. Repeat at 320x568, 390x844, 844x390, and desktop without horizontal overflow.
26. In Stage Audio, start the program and confirm the Pod reports `PROGRAM BUS` then `program mix` without requesting camera permission. Toggle Voice, Program, Atmos, Monitor, and Master; verify their state attributes and meters, and confirm master mute silences both the program bus and the controlled Stage microphone. From Nexus > Pod, choose Podcast, open Stage Output, verify the new tab uses the Nexus room and Audio console, then check the matching `/watch/:roomCode` viewer before starting protected RTMP egress.
27. On phones, open Audio > Phone Listener or the room's Public Viewer. Press Listen Live once to satisfy iOS and Android playback rules; the control remains available until LiveKit confirms speaker playback. The viewer reports when sound is unlocked but the room mix has not arrived yet.
28. In the phone viewer, confirm Listen Live reports venue and live-room sound, then switch between Program and Venue. The Program view must show the direct remote feed and the Venue view must keep the same feed moving on the center Three.js screen. The hidden viewer health element exposes venue audio levels and the active program feed for diagnostics.
