...
import { h3atAgents, h3atEventLadder, h3atRemoteControls, h3atSimulationStages, h3atTracks } from "../h3at-partnership";
...
export function H3ATPartnerPage() {
  ...
  return <div className="page section-wrap h3at-partner-page">
    ...
    <section className="h3at-section"><div className="section-heading"><div><span className="eyebrow">PROJECTS TO PLACEMENT</span><h2>Community showcase ladder</h2></div><StatusPill tone="green">REAL PROJECTS</StatusPill></div><div className="h3at-event-ladder">{h3atEventLadder.map((event, index) => <article key={event.cadence}><span>{String(index + 1).padStart(2, "0")}</span><small>{event.cadence}</small><h3>{event.title}</h3><p>{event.purpose}</p><b><ShieldCheck/>{event.gate}</b></article>)}</div></section>
