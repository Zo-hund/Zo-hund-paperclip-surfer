...
import {
  ...
  Pause, Play, Plus, QrCode, RefreshCw, Rocket, Save, Settings2, ShieldCheck, Target, Trash2, Users, X,
} from "lucide-react";
...
function BrandWorkspace({ organization, canManage, onSaved, onError }: { organization: PartnerOrganization; canManage: boolean; onSaved: (organization: PartnerOrganization) => void; onError: (message: string) => void }) {
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setBusy(true); try { const url = await uploadPartnerLogo(organization.id, file); const saved = await updatePartnerOrganization(organization.id, { ...draft, logo_url: url }); setDraft(saved); onSaved(saved); } catch (error) { onError(messageOf(error)); } finally { setBusy(false); event.target.value = ""; } };
  const removeLogo = async () => { setBusy(true); try { const saved = await updatePartnerOrganization(organization.id, { ...draft, logo_url: null }); setDraft(saved); onSaved(saved); } catch (error) { onError(messageOf(error)); } finally { setBusy(false); } };
  return <form className="partner-brand" onSubmit={save}><div className="partner-section-head"><div><span className="eyebrow">IDENTITY + PROOF</span><h3>Partner runtime configuration</h3></div>{canManage && <button className="button primary" disabled={busy}><Save/>Save configuration</button>}</div>
    <div className="partner-brand-layout"><div className="partner-brand-preview" style={{ "--partner": draft.brand_color } as React.CSSProperties}><div>{draft.logo_url ? <img src={draft.logo_url} alt="Partner logo preview"/> : draft.name.slice(0, 2).toUpperCase()}</div><span className="eyebrow">VERIFIED PARTNER</span><h2>{draft.name}</h2><p>{draft.certificate_name}</p><code>{draft.proof_scope}</code>{canManage && <><small className="partner-logo-help">PNG, JPEG, or WebP / 5 MB maximum</small><span className="partner-logo-actions"><label className={`button secondary ${busy ? "disabled" : ""}`}><ImageUp/>{busy ? "Saving..." : draft.logo_url ? "Replace logo" : "Upload logo"}<input disabled={busy} type="file" accept="image/png,image/jpeg,image/webp" onChange={upload}/></label>{draft.logo_url && <button type="button" className="icon-button" title="Remove organization logo" aria-label="Remove organization logo" disabled={busy} onClick={() => void removeLogo()}><Trash2/></button>}</span></>}</div>
