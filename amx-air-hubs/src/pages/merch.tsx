...
function MerchProductCard({ product, onAdd }: { product: MerchProduct; onAdd: (product: MerchProduct, variantId: string) => void }) {
  ...
  return <article className="merch-product"><div className={`merch-product-image product-${product.id}`}><img src={product.imageUrl} alt={product.name}/><span>{product.collectiveSharePercent}% TO {product.partnerName.toUpperCase()}</span></div><div><span className="eyebrow">{product.category}</span><h3>{product.name}</h3><p>{product.description}</p><label>Variant<select value={variantId} onChange={(event) => setVariantId(event.target.value)}>{product.variants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><footer><b>{money(variant?.priceCents || 0, variant?.currency)}</b><button className="button primary" disabled={!variant?.available} onClick={() => onAdd(product, variantId)}><Plus/>Add</button></footer></div></article>;
}
