...
import {
  ...
  ListChecks, Megaphone, Mic2, Minus, MonitorPlay, Music2, Pause, Play, Plus, Podcast, Radio, RadioTower, RefreshCw, Smartphone, Sparkles, Users, Video, Volume2, Wifi, X, Zap,
} from "lucide-react";
...
export function AMXXRStagePage() {
  ...
  const quickTakeFeed = (route: string) => {
    ...
    production.update({ cameraRoutes: { ...production.state.cameraRoutes, [shot]: route }, shot, cameraMotion: { ...production.state.cameraMotion, id: "static", loop: false, startedAt: null } });
    trackEvent("stage_camera_quick_take", { campaignId: route, locationTag: production.room });
  };
  ...
  const updateProgramMedia = (programMedia: typeof production.state.programMedia) => production.update({ programMedia });
      ...
      <aside className="stage-console">
        ...
        <div ref={consoleBodyRef} className="stage-console-body">
          ...
          <div className="stage-console-view" hidden={view !== "production"}>
            ...
            {orderedVideoFeeds.length > 0 && <section className="stage-control-section stage-live-input-bank"><header><div><span className="eyebrow">LIVE INPUT BANK</span><h2>Hot camera switching</h2></div><small>QUICK CUT TO {programChannel.label}</small></header><div>{orderedVideoFeeds.map((feed) => <button key={feed.id} className={programChannel.feed?.id === feed.id ? "active" : ""} onClick={() => quickTakeFeed(feed.id)}><span className="stage-camera-preview"><CameraFeedPreview feed={feed}/></span><span><b>{feed.name}</b><small>{feed.source}{feed.muted ? " / MUTED" : " / READY"}</small></span><Zap/></button>)}</div></section>}
            <StageProgramMediaDeck media={production.state.programMedia} tenantId={tenantId} onUpdate={updateProgramMedia}/>
