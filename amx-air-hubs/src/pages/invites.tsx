...
export function PodInvitePage() {
  ...
  const accept=async()=>{if(!invite)return;if(!member.session){navigate(`/account?next=${encodeURIComponent(invite.joinPath)}`);return}setBusy(true);setError("");try{const accepted=await acceptPodInvite(token);const mission=missions.find((item)=>item.id===accepted.missionId)||missions[0];setActiveMissionId(mission.id);joinImmersiveRoomFromInvite(accepted,role);navigate(podInviteDestination(accepted))}catch(reason){setError(reason instanceof Error?reason.message:"This invite could not be accepted.");setBusy(false)}};
