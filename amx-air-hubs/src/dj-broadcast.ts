...
export async function controlDjBroadcast(action: "status" | "start" | "stop", room: string, controlToken: string, egressId?: string) {
  const response = await fetch(`/api/livekit/egress/dj/${action}`, {
    method: "POST",
    headers: { "Authorization": `Bearer ${controlToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ room, ...(egressId ? { egressId } : {}) }),
  });
