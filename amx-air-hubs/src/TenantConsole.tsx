...
import {
  Bot, Building2, Check, FileBadge, Plus, Radio, Save, School, ShieldCheck, WalletCards, X,
} from "lucide-react";
...
export function TenantConsolePage() {
  ...
  return <div className="page section-wrap">
    ...
    <aside className="tenant-detail" style={{ "--tenant": current.color } as CSSProperties}><span className="eyebrow">ACTIVE TENANT</span><h2>{current.name}</h2><StatusPill tone="green">Scoped</StatusPill><div className="tenant-facts"><span><Building2/>ORGANIZATION<b>{current.type}</b></span><span><Radio/>MISSIONS<b>{current.missionIds.length} enabled</b></span><span><Bot/>AGENTS<b>{current.agentIds.map((id) => agents.find((agent) => agent.id === id)?.name || id).join(" / ")}</b></span><span><ShieldCheck/>PROOF SCOPE<b>{current.proofScope}</b></span></div><button className="button secondary full" onClick={() => openEditor(current)}>Edit tenant configuration</button></aside></div>
