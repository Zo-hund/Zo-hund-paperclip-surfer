...
import { absoluteInviteUrl, createPodInvite, getOwnedInviteZkode, getOwnedPodInvite, resolvePodInvite, revokePodInvite, storeOwnedPodInvite, type PodInvite, type PodInviteRole } from "./pod-invites";
...
export function PodInvitePanel({pod,tenant,compact=false}:PodInvitePanelProps) {
  ...
  return <section id="pod-showcase-invite" className={`pod-invite-panel ${compact?"compact":""}`}>
    ...
    </div>:<div className="pod-invite-ready"><div className="pod-invite-qr"><QRCodeCard route={invite.joinPath} title={invite.title}/></div><div className="pod-invite-summary"><span><ShieldCheck/>QR + ZKODE required to unlock</span><span><Users/>{invite.useCount} / {invite.maxUses} accepted</span><span><CalendarClock/>Expires {new Date(invite.expiresAt).toLocaleString()}</span><code>{absoluteInviteUrl(invite)}</code><code>ZKODE: {getOwnedInviteZkode(invite.token)||"LEGACY PASS - REISSUE"}</code><div><button className="button secondary" onClick={copy}><Copy/>Copy link</button><button className="button primary" onClick={share}><Send/>Share invite</button><button className="icon-button danger" aria-label="Revoke invite" title="Revoke invite" disabled={busy||invite.status!=="active"} onClick={revoke}><Trash2/></button></div></div></div>}
