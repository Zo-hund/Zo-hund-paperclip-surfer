...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.deploymentMode === "local_trusted" && !isLoopbackHost(config.host)) {
    throw new Error(
      `local_trusted mode requires a loopback host binding (received: '${config.host}'). ` +
        "Binding to '0.0.0.0' or any non-loopback address in local_trusted mode exposes an " +
        "unauthenticated admin surface to the network. Use authenticated mode for non-loopback deployments.",
    );
  }
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    setInterval(() => {
      ...
      void heartbeat
        ...
        .then((result) => {
          if (result.enqueued > 0) {
            logger.info({ ...result }, "agent cron scheduler tick enqueued runs");
          }
        })
        .catch((err) => {
          logger.error({ err }, "agent cron scheduler tick failed");
        });
