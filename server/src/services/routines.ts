...
import { parseCron, validateCron, nextCronTickInTimeZone as _nextCronTickInTimeZone, assertValidTimeZone } from "./cron.js";
...
/** Wrap shared assertValidTimeZone to throw an HTTP unprocessable error. */
...
/**
 * Timezone-aware next cron tick — wraps the shared implementation but converts
 * validation errors to HTTP 422 responses for use in route handlers.
 */
function nextCronTickInTimeZone(expression: string, timeZone: string, after: Date) {
  assertTimeZone(timeZone);
  const error = validateCron(expression.trim());
  if (error) throw unprocessable(error);
  return _nextCronTickInTimeZone(expression, timeZone, after);
}
