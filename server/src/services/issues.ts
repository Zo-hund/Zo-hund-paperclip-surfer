  ...
  return {
    ...
    listComments: async (
      ...
      if (afterCommentId) {
        ...
        if (!anchor) return [];
        const createdAtStr = anchor.createdAt.toISOString();
        conditions.push(
          order === "asc"
            ? sql<boolean>`(
                ${issueComments.createdAt} > ${createdAtStr}
                OR (${issueComments.createdAt} = ${createdAtStr} AND ${issueComments.id} > ${anchor.id})
              )`
            : sql<boolean>`(
                ${issueComments.createdAt} < ${createdAtStr}
                OR (${issueComments.createdAt} = ${createdAtStr} AND ${issueComments.id} < ${anchor.id})
