...
export function kpiAnalyticsService(db: Db) {
  return {
    ...
    /**
     * Phase F: lists OPPRRC completion reports (markdown documents generated by
     * opprrcService.generateReport) for the company, newest first.
     */
    async listOpprrcReports(companyId: string) {
      const rows = await db
        .select({
        ...
        .where(and(eq(issueWorkProducts.companyId, companyId), eq(issueWorkProducts.type, "document")))
        .orderBy(desc(issueWorkProducts.createdAt));

      return rows
        .filter((r) => (r.metadata as Record<string, unknown> | null)?.format === "markdown")
        .map((r) => {
          ...
          return {
            ...
            title: r.title,
            summary: r.summary,
            generatedAt: (metadata.generatedAt as string | undefined) ?? r.createdAt.toISOString(),
          };
        });
    },
