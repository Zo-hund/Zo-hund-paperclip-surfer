...
export function buildOpenRouterTaskPrompt(context: Record<string, unknown>): string {
  ...
  return [
    `Current authoritative wake context: ${JSON.stringify(buildOpenRouterWakeEnv(context))}`,
    ...
    "Use paperclip_api for all platform calls including memories, with method, path starting /api/, and a structured JSON body for mutations. The tool supplies authentication, Content-Type and run audit headers from the runtime. Never use curl or shell JSON for platform calls; never print secrets or supply credentials.",
    ...
    "The runtime independently verifies assigned-task ownership, in_review/done status and a registered readable primary platform document before accepting completion. For external or non-document outputs, also register a primary platform document recording the actual verification evidence and output links; external URLs are not automatically fetched. A system-generated report does not satisfy delivery.",
