 ...
 */
export const amxChainEvents = pgTable(
  "amx_chain_events",
  {
 ...
 */
export const amxCertificates = pgTable(
  "amx_certificates",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    ...
    commitHashes: jsonb("commit_hashes").$type<string[]>().notNull().default([]),
    ...
    projects: jsonb("projects").$type<string[]>().notNull().default([]), // OPPRRC mappings
    resources: jsonb("resources").$type<string[]>().notNull().default([]), // OPPRRC mappings
    reports: jsonb("reports").$type<string[]>().notNull().default([]), // OPPRRC mappings
