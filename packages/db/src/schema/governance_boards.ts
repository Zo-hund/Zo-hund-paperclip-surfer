 ...
 */
export const collectives = pgTable(
  "collectives",
  {
    ...
    skillProviders: jsonb("skill_providers").$type<string[]>().notNull().default([]), // Links to users/agents
 ...
 */
export const electives = pgTable(
  "electives",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    ...
    authorizedPrincipals: jsonb("authorized_principals").$type<string[]>().default([]), // Who can hire/buy
    status: text("status").notNull().default("active"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
...
export const communityBoards = pgTable(
  "community_boards",
  {
    ...
    activeNeeds: jsonb("active_needs").$type<string[]>().default([]), // Requirements linked to this board
    status: text("status").notNull().default("active"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
 ...
 */
export const boardCollaborations = pgTable(
  "board_collaborations",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
