...
export const agentKpiDefinitions = pgTable("agent_kpi_definitions", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
...
export const agentExperiments = pgTable("agent_experiments", {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id").notNull().references(() => agents.id, { onDelete: "cascade" }),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    ...
    kpiResultsA: jsonb("kpi_results_a").$type().notNull().default({}),
    kpiResultsB: jsonb("kpi_results_b").$type().notNull().default({}),
...
export const agentKpiObservations = pgTable("agent_kpi_observations", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    observerType: text("observer_type").notNull().$type(),
    observerAgentId: uuid("observer_agent_id").references(() => agents.id),
    observerUserId: uuid("observer_user_id"),
    observation: text("observation").notNull(),
    agentIds: jsonb("agent_ids").$type().notNull().default([]),
...
//# sourceMappingURL=agent_kpis.js.map