 ...
 */
export const rqSubmissions = pgTable("rq_submissions", {
    ...
    contextData: jsonb("context_data").$type().notNull(),
    deploymentMode: text("deployment_mode").notNull().default("online"), // 'in_person', 'online', 'metaverse'
    agentSwarmIds: jsonb("agent_swarm_ids").$type().default([]), // The Strategy/Visual/Spatial/Doc/Content swarm
 ...
 */
export const rqAgentConfigs = pgTable("rq_agent_configs", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
...
//# sourceMappingURL=rq_factory.js.map