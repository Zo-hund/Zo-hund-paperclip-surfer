 ...
 */
export const lmsWorkshops = pgTable("lms_workshops", {
    ...
    schedule: jsonb("schedule").$type(), // Morning/Afternoon/Evening/Night slots
...
export const lmsEnrollments = pgTable("lms_enrollments", {
    ...
    score: integer("score"),
    certificatesAwarded: jsonb("certificates_awarded").$type().default([]), // Links to AMX Certificates
 ...
 */
export const lmsSimulations = pgTable("lms_simulations", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id),
    name: text("name").notNull(),
...
//# sourceMappingURL=lms_training.js.map