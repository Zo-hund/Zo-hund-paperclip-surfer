...
## Verified-link presentation repair

The candidate now returns verified task ID, persisted status and normalized relative registered document URL from the delivery verifier. Successful assigned run summaries use fixed-label links derived only from those verified results, and `resultJson.kind = verified_deliveries` supplies the same structured records. Generated model prose remains in conversation history and is labeled unverified in logs; it cannot replace the verified summary. Failures retain their error summary and publish no successful delivery metadata, even when an earlier task verified successfully. Unassigned informational behavior is unchanged.

The parent reported the frozen full suite immediately before this presentation-only change: **201 files passed, one skipped; 1,229 tests passed, one skipped; exit 0; 730.21 seconds**, recorded in `candidate-verified-full-tests.log`. That full-suite result applies to the source before this final formatting change. After the change, **52 focused tests across four files passed** in 1.79 seconds, and the OpenRouter adapter TypeScript compilation/build exited 0. Regressions cover generated wrong origins, relative normalization, retained model history, and failure after partial verification. No additional provider run, commit, push or production deployment was performed for this presentation repair; final-head CI remains the parent's release validation step.
