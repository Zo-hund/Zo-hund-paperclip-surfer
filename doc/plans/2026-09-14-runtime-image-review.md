...
The subsequent [candidate CI run](https://github.com/Zo-hund/Zo-hund-paperclip-surfer/actions/runs/34925540064), including the authorization, Codex isolation, memory and cache repairs, also passed image build and isolated runtime smoke, then failed Trivy with the same 224 OS and nine application findings. Both smoke jobs, build/test and Helm validation passed. Signing and release metadata were skipped. This run predates the native API delivery verifier; it is not validation of that later source. Local log: `candidate-latest-image-scan.log`. No production change followed.
